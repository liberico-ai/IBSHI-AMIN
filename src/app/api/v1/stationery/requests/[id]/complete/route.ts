...
// Hoàn thành phiếu yêu cầu — chuyển sang COMPLETED + trừ tồn kho.
// Chỉ HCNS staff tạo phiếu mới được bấm Hoàn thành (sau khi đã APPROVED).
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (req.createdById !== userId && !["HR_ADMIN", "BOM"].includes((session.user as any).role))
