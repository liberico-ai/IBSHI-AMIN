...
export async function GET(request: NextRequest) {
    ...
    return NextResponse.json({
      data: {
        ...
        departments: departments.map((d) => ({
          id: d.id, name: d.name, headcount: d.headcount, actual: d._count.employees,
        })),
  ...
  if (type === "attendance") {
    ...
    for (const emp of employees) {
      empMap[emp.id] = { ...emp, present: 0, absent: 0, late: 0, otHours: 0 };
    }
  ...
  if (type === "weekly-hr") {
    ...
    if (!weekStartStr) {
      return NextResponse.json({ error: { code: "VALIDATION_ERROR", message: "weekStart required" } }, { status: 400 });
    }
    ...
    const [active, totalEmp, newHires, resigned, attByStatus, leaveByStatus, otAgg, employees] = await Promise.all([
      ...
      prisma.employee.findMany({
        ...
        orderBy: [{ department: { sortOrder: "asc" } }, { fullName: "asc" }],
      }),
    ]);
    ...
    return NextResponse.json({
      data: {
        ...
        attendance: employees.map((e) => ({
          ...
          ...(attMap[e.id] || { present: 0, absent: 0, late: 0, halfDay: 0 }),
        })),
        leave: {
        ...
        ot: {
          count: otAgg._count.id,
          totalHours: otAgg._sum.hours || 0,
        },
      },
    });
  }
  ...
  if (type === "monthly-hr") {
    ...
    const [
      totalEmp, activeEmp, probationEmp, newHires, resigned,
      departments, attByDeptRaw, leaveByType, payrollPeriod,
      trainingPlans, trainingCompleted, hseIncidents, hseNearMiss, disciplines,
    ] = await Promise.all([
      prisma.employee.count(),
      prisma.employee.count({ where: { status: "ACTIVE" } }),
      prisma.employee.count({ where: { status: "PROBATION" } }),
      ...
      prisma.department.findMany({
        where: { isActive: true },
        select: {
          name: true, code: true, headcount: true,
      ...
      prisma.leaveRequest.groupBy({
        by: ["leaveType", "status"],
        where: { startDate: { gte: from, lte: to }, status: "APPROVED" },
        _count: true,
        _sum: { totalDays: true },
      }),
      prisma.payrollPeriod.findFirst({
        ...
        include: {
          records: {
            select: {
              grossSalary: true, netSalary: true, bhxh: true, bhyt: true, bhtn: true, tncn: true,
    ...
    const payroll = payrollPeriod ? {
      ...
      byEmployee: payrollPeriod.records.map((r) => ({
    ...
    return NextResponse.json({
      data: {
        ...
        departments: departments.map((d) => ({
          code: d.code, name: d.name, planned: d.headcount, actual: d._count.employees,
        })),
        attendance: {
          present: attMap["PRESENT"] || 0,
          absent: (attMap["ABSENT_UNAPPROVED"] || 0) + (attMap["ABSENT_APPROVED"] || 0),
          late: attMap["LATE"] || 0,
  ...
  if (type === "finance-summary") {
    ...
    const [payrollPeriod, vehicleBookings, mealRegs] = await Promise.all([
      ...
      prisma.mealRegistration.aggregate({
        where: { date: { gte: from, lte: to } },
        _sum: { lunchCount: true, dinnerCount: true, guestCount: true },
      }),
    ]);
    ...
    return NextResponse.json({
      data: {
        ...
        payroll: {
          ...
          employees: payrollRecords.map((r) => ({
            code: r.employee.code,
            fullName: r.employee.fullName,
            department: r.employee.department.name,
            grossSalary: r.grossSalary,
        ...
        vehicles: {
          total: vehicleBookings.reduce((s, v) => s + v._count, 0),
          approved: vehicleMap["APPROVED"] || 0,
          completed: vehicleMap["COMPLETED"] || 0,
        },
        meals: {
          lunch: lunchTotal,
          dinner: dinnerTotal,
          guest: guestTotal,
          total: lunchTotal + dinnerTotal + guestTotal,
        },
      },
    });
  }
  ...
  if (type === "dashboard-kpi") {
    ...
    const [
      ...
      prisma.payrollPeriod.findFirst({ where: { month: now.getMonth() + 1, year: now.getFullYear() } }),
    ]);
